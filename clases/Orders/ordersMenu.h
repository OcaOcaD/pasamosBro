...
int stringToIntOrder( string num );
string intToStringOrder( int num );
//Utilities
...
// string ltrim(const string& s){
// 	size_t start = s.find_first_not_of(WHITESPACESS);
// 	return (start == string::npos) ? "" : s.substr(start);
// }
// string rtrim(const string& s){
// 	size_t end = s.find_last_not_of(WHITESPACESS);
...
void ordersMenu() {
    ...
    do{
        ...
        if( last != 0 ){
            ...
            id = stringToIntOrder( orders[last-1].get_id() );  //Last id in int
            id = last;
        }else{
            id = 0;
        }
...
Order ask4Order( int last_id ){
    ...
    actual_id = intToStringOrder( last_id );
